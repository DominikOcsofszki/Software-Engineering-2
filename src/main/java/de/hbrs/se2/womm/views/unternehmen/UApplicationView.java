...
public class UApplicationView extends AViewWomm implements HasUrlParameter<String> {
    ...
    public void setParameter(BeforeEvent beforeEvent, @OptionalParameter String bewerbungID) {
        if (bewerbungID == null || bewerbungID.equals("example")) {
            ...
            try {
                ...
                var h1 = getWommBuilder().H1.create("%s is not a valid ID.");
                h1.setText(String.format(h1.getText(),bewerbungID));
                add(h1);
            }
        }
    }
    ...
    void setUpExample() {
        ...
        top.add(image);
        var name = getWommBuilder().H2.create("Surname, Name");
        top.add(name);
        Button chat = new Button(getWommBuilder().translateText("Chat"), event -> {
        ...
        var details = new Details(getWommBuilder().translateText("Motivational letter"), anschreiben);
    ...
    void setUpNotFound(String id) {
        var curr = getWommBuilder().H1.create("The application with the ID '%s' could not be found.");
        curr.setText(String.format(curr.getText(),id));
        add(curr);
        //add(new H1(String.format(getWommBuilder().translateText(), id)));
    }
